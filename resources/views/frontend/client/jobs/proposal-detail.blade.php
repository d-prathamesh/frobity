        ...
        <div class="row">
         <div class="col-lg-12 col-md-12 col-sm-6 col-xs-12">
          ...
          <div class="col-lg-12 top-sec">
		   ...
		   //print_r($proposal);
		   //echo "</pre>";
		   $attach_file_id = $proposal['fileattachments']['file_id'] ;
		   $search = 'https://frobity.com/storage/attachments' ;
           $trimmed_file_id = str_replace($search, '', $attach_file_id) ;
           //echo count($trimmed_file_id) ;
		   
		   ?>
           <p>{{ $proposal['proposal_summary'] ? $proposal['proposal_summary'] : 'N/A'}}</p>
           
			<h3>Attachement</h3>
			@if(empty($trimmed_file_id))
				<p>N/A</p>
			@else
			<p><a style="color:black;" href="{{ url('/attachments'. $trimmed_file_id) }}" class="card-link" target="_blank"  download><i class="fa fa-files-o"></i>  {{$proposal['fileattachments']['file_name']}}</a></p>
			@endif
		   ...
		   <div >
			...
			<a class="kafe-btn kafe-btn-mint-small" href="{{route('web.client.decline',['id'=>$proposal['job']['id'],'proposalId'=>$proposal['id']])}}">
				Decline
     		</a>			
			<!--<a class="kafe-btn kafe-btn-mint-small btn-danger">View Other Proposals</a>
			<a class="kafe-btn kafe-btn-mint-small btn-danger" href="{{ route('web.client.jobs.proposals',['id'=>$proposal['job']['id']]) }}">
...
<script src="https://checkout.razorpay.com/v1/checkout.js"></script>
    <script>
      var options = {
        ...
        "amount":'{{isset($proposal['amount'])?$proposal['amount']*100:0}}',
        "payment_capture":1,

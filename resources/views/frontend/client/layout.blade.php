...
/*				padding: 0px 0px 0px 4px;
		 	...
		 	<li class="topnav-notifications"><i class="fa fa-envelope fa-2x dropdown-toggle"  onclick="ShowMessage()" data-toggle="dropdown" aria-expanded="true"><span class="message_no badge badge-light dropdown-toggle" style="cursor:pointer"></span></i>
			  ...
			  <!--<div class="dropdown-menu w dropdown-menu-scale pull-right">
				<a class="dropdown-item" href="#">No new Messages</a>		   
			  		...
			  		<div class="no-message" style="padding:9px;">No new Messages</div>
					
					<ul id="message-list" style="overflow-y:auto;overflow-x:hidden;width:auto; max-height:400px;">
			  			
			  		</ul>
			  		<ul id="message-list" style="width:auto;">
						<li><a href="{{ route('web.client.messages') }}">Show all Messages</a></li>
			  		</ul>
	  ...
	  <div class="container-fluid">
	   <div class="row">
		...
		<ul class="sidebar-menu" data-widget="tree">
			...
			<li class="{{ Session::get('currentRoute') == 'web.client.transactions' ? 'active' : ''}}">
			  <a href="{{ route('web.client.transactions') }}">
				<i class="fa fa-user"></i> <span>Transactions</span>
			  </a>
			</li>
		</ul>	
...
//		notification check 
	...
	function ShowNotification(){
		...
		$.ajax({
			...
			success:function( response ){
				...
				$("#notification-list").html(noti_html);
				$(".icon_no").hide();
			}
		});
	}
	...
	function checkMessages(){
		$.ajax({
			url:'{{ route('web.client.messages') }}',
			success:function( response ){
				console.log( response );
				response_json = response;//JSON.parse(response);
				newmessages = response_json.new_messages;
				console.log(newmessages);
				if( newmessages > 0 ){
					$(".message_no").html(newmessages);
				}
				setTimeout( checkMessages, 2000 );
			}
		});
	...
	function ShowMessage(){
		...
		$.ajax({
			...
			success:function( response ){
				console.log( response );
				response_json = response;//JSON.parse(response);
				newmessages = response_json.result;
				console.log(newmessages);
				...
				message_html= '';
				for( ln in newmessages ){
					var url = "{{route('web.client.ongoing.jobs.detail', '')}}"+"/"+newmessages[ln]['job_id']+"-"+newmessages[ln]['job_application_id'];
					//noti_html = noti_html + '<li>'+newleads[ln]['notification_title']+'</li>';
						//message_html = message_html + '<li>'+newmessages[ln]['message']+'</li><br>';
						message_html = message_html + '<a href='+url+'><li>'+newmessages[ln]['message']+"-"+"Job Title-  "+newmessages[ln]['job_title']+'</li><br></a>';
				}
				message_html = message_html ;// + '<a href="#"><li>Show ALL</li></a>';
